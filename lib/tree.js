...
export class TreeNodeIterator extends Iterator {
    ...
    if (this.nextNode.isLeaf()) {
      if (this.nextNode.isRoot()) {
        ...
        do {
          ...
          const nextSibling = currentNode.iterator().checkAndGetRightSiblingNode();
          if (nextSibling !== null) {
  ...
  remove() {
    const errorMessage = 'Failed to remove the tree node. ';
    ...
    if (this.currentNode.isRoot()) {
      const message = `${errorMessage}The tree node ${this.currentNode}s is root`;
    ...
    while (true) {
      ...
      const rightSiblingNode = currentNode.iterator().checkAndGetRightSiblingNode();
      if (rightSiblingNode !== null) {
...
export class TreeNode {
  constructor({
    data = null, add = null, clear = null, dropSubtree = null, iterator = null, subtrees = null,
  } = {}) {
    this.parent = null;
    this.data = data;
  ...
  commonAncestor(node = null) {
    const errorMessage = 'Unable to find the common ancestor between tree nodes. ';
    if (node === null) {
      const message = `${errorMessage}The specified tree node is null`;
      throw new ReferenceError(`TreeNodeException ${message}`);
    }
    ...
    if (this.isRoot()
            || node.isRoot()) {
      const message = `${errorMessage}The tree node ${this.isRoot() ? this : node}s is root`;
      throw new ReferenceError(`TreeNodeException ${message}`);
    }
    if (this === node
    ...
    const thisNodeLevel = this.level();
    const thatNodeLevel = node.level();
  ...
  find(data = null) {
    ...
    this.traversePreOrder(new TraversalAction({
      ...
      isCompleted: () => searchedNode[0] !== null,
  ...
  path(descendant = null) {
    ...
    const errorMessage = 'Unable to build the path between tree nodes. ';
    if (descendant.isRoot()) {
      const message = `${errorMessage}Current node ${descendant}s is root`;
      throw new ReferenceError(`TreeNodeException ${message}`);
    }
    const path = [];
    let node = descendant;
  ...
  postOrdered() {
    ...
    const mPostOrdered = [];
    const action = TreeNode.populateAction(mPostOrdered);
  ...
  preOrdered() {
    ...
    const mPreOrdered = [];
    const action = TreeNode.populateAction(mPreOrdered);
  ...
  removeAll(nodes = null) {
    ...
    let result = false;
    for (const node of nodes) {
      const currentResult = this.remove(node);
  ...
  size() {
    ...
    const count = [0];
    const action = new TraversalAction({
  ...
  toString() {
    let builder = '\n';
    const topNodeLevel = this.level();
    const action = new TraversalAction({

      perform: (node) => {
        const nodeLevel = node.level() - topNodeLevel;
...
export class MultiTreeNode extends TreeNode {
  ...
  dropSubtrees(subtrees) {
    ...
    let result = false;
    for (const subtree of subtrees) {
      const currentResult = this.dropSubtree(subtree);
      if (!result && currentResult) {
        result = true;
      }
    }
  ...
  siblings() {
    if (this.isRoot()) {
      const message = `"Unable to find the siblings. The tree node ${this.root()}s is root`;
      throw new ReferenceError(`TreeNodeException ${message}`);
    }
    const parentSubtrees = this.parent.subtrees();
    const parentSubtreesSize = parentSubtrees.length;
...
export class ArrayMultiTreeNode extends MultiTreeNode {
  ...
  addSubtrees(subtrees) {
    ...
    const subtreesArray = subtrees;
    const subtreesArrayLength = subtreesArray.length;
  ...
  clear() {
    if (!this.isLeaf()) {
      for (let i = 0; i < this.subtreesSize; i++) {
        const subtree = this.mySubtrees[i];
  ...
  dropSubtree(subtree) {
    ...
    const mSubtreeIndex = this.indexOf(subtree);
    if (mSubtreeIndex < 0) {
      return false;
    }
    const mNumShift = this.subtreesSize - mSubtreeIndex - 1;
    if (mNumShift > 0) {
  ...
  hasSubtree(subtree = null) {
    ...
    for (let i = 0; i < this.subtreesSize; i++) {
      const mSubtree = this.mySubtrees[i];
  ...
  increaseSubtreesCapacity(minSubtreesCapacity) {
    const oldSubtreesCapacity = this.mySubtrees.length;
  ...
  iterator() {
    return new TreeNodeIterator({
      ...
      leftMostNode: () => this.mySubtrees[0],

      rightSiblingNode: () => {
        const mParent = this.parent;
        const rightSiblingNodeIndex = mParent.indexOf(this) + 1;
  ...
  siblings() {
    ...
    const mParent = this.parent;
    const parentSubtreesSize = mParent.subtreesSize;
    if (parentSubtreesSize == 1) {
      return [];
    }
    const parentSubtreeObjects = mParent.subtrees;
    const siblings = new Array(parentSubtreesSize - 1);
    for (let i = 0; i < parentSubtreesSize; i++) {
      const parentSubtree = parentSubtreeObjects[i];
...
export class LinkedMultiTreeNode extends MultiTreeNode {
  ...
  clear() {
    if (!this.isLeaf()) {
      ...
      while (nextNode !== null) {
        ...
        const nextNodeRightSiblingNode = nextNode.rightSiblingNode;
  ...
  iterator() {
    return new TreeNodeIterator({
      ...
      leftMostNode: () => this.leftMostNode,
  ...
  siblings() {
    ...
    const firstNode = this.parent().leftMostNode;
