...
export class ArrayMap {
    ...
    ensureCapacity(minimumCapacity) {
        if (this.mHashes.length < minimumCapacity) {
            let ohashes = this.mHashes;
            let oarray = this.mArray;
    ...
    indexOf(key, hash) {
        ...
        let index = util.binarySearch(this.mHashes, N, hash);
    ...
    indexOfNull() {
        ...
        let index = util.binarySearch(this.mHashes, N, 0);
        // If the hash code wasn't found, then we have no entry for this key.
        if (index < 0) {
            return index;
        }
    ...
    put(key, value) {
        ...
        if (index >= 0) {
            index = (index << 1) + 1;
            let old = this.mArray[index];
            this.mArray[index] = value;
            return old;
        }
        index = ~index;
        if (this.mSize >= this.mHashes.length) {
            let n = this.mSize >= (BASE_SIZE * 2) ? (this.mSize + (this.mSize >> 1))
                : (this.mSize >= BASE_SIZE ? (BASE_SIZE * 2) : BASE_SIZE);
...
export class SparseArray {
    ...
    removeAtRange(index, size) {
        let end = Math.min(this.mSize, index + size);
